...
export function PrayerTimesWidget() {
  // 1. Initialize state from LocalStorage to prevent the "Calculating" flash
  const [timings, setTimings] = useState<PrayerTimings | null>(() => {
    const saved = localStorage.getItem("cached_prayer_times");
    return saved ? JSON.parse(saved) : null;
  });
  
  const [loading, setLoading] = useState(!timings); // Only load if we have no cache
  ...
  // 3. UI logic: Show skeleton ONLY if we have absolutely no data and are loading
